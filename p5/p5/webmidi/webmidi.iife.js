...
(function (exports) {
		...
		removeListener(event, callback, options = {}) {
			...
			let listeners = this.eventMap[event].filter(listener => {
				return (
					...
					(options.remaining &&
						options.remaining !== listener.remaining) ||
			...
			return new Promise((resolve, reject) => {
				...
				if (options.duration !== Infinity) {
					timeout = setTimeout(() => {
						listener.remove();
						reject(
							"The duration expired before the event was emitted."
						);
					}, options.duration);
				}
			});
	...
	class Utilities {
		...
		static toNoteNumber(identifier, octaveOffset = 0) {
			// Validation
			octaveOffset =
				octaveOffset == undefined ? 0 : parseInt(octaveOffset);
		...
		static getChannelModeByNumber(number) {
			...
			for (let cm in Enumerations.MIDI_CHANNEL_MODE_MESSAGES) {
				if (
					Enumerations.MIDI_CHANNEL_MODE_MESSAGES.hasOwnProperty(
						cm
					) &&
	...
	class OutputChannel extends EventEmitter {
		...
		sendKeyAftertouch(target, pressure, options = {}) {
			...
			if (!options.rawValue)
				pressure = Utilities.fromFloatTo7Bit(pressure); // Plot total offset
			...
			Utilities.buildNoteArray(target).forEach(n => {
				this.send(
					[
						(Enumerations.MIDI_CHANNEL_MESSAGES.keyaftertouch <<
							4) +
							(this.number - 1),
		...
		sendControlChange(controller, value, options = {}) {
			if (typeof controller === "string") {
				controller =
					Enumerations.MIDI_CONTROL_CHANGE_MESSAGES[controller];
			}
			...
			value.forEach((item, index) => {
				this.send(
					[
						(Enumerations.MIDI_CHANNEL_MESSAGES.controlchange <<
							4) +
							(this.number - 1),
						controller + index * 32,
						value[index],
		...
		sendRpnDecrement(parameter, options = {}) {
			...
			if (wm.validation) {
				if (parameter === undefined) {
					throw new TypeError(
						"The specified registered parameter is invalid."
					);
				}
				...
				Object.getOwnPropertyNames(
					...
					if (
						Enumerations.MIDI_REGISTERED_PARAMETERS[p][0] ===
							parameter[0] &&
						Enumerations.MIDI_REGISTERED_PARAMETERS[p][1] ===
							parameter[1]
					) {
						valid = true;
					}
		...
		sendChannelMode(command, value = 0, options = {}) {
			...
			if (wm.validation) {
				if (command === undefined) {
					throw new TypeError(
						"Invalid channel mode message name or number."
					);
				}
		...
		sendChannelAftertouch(pressure, options = {}) {
			...
			this.send(
				[
					(Enumerations.MIDI_CHANNEL_MESSAGES.channelaftertouch <<
						4) +
						(this.number - 1),
		...
		sendProgramChange(program, options = {}) {
			...
			if (wm.validation) {
				if (!(program >= 0 && program <= 127)) {
					throw new RangeError(
						"The program number must be between 0 and 127."
					);
				}
			}
	...
	class Output extends EventEmitter {
		...
		send(
			...
			if (wm.validation) {
				...
				if (
					!(
						parseInt(message[0]) >= 128 &&
						parseInt(message[0]) <= 255
					)
				) {
					throw new RangeError(
		...
		sendSysex(identification, data = [], options = {}) {
			...
			if (data instanceof Uint8Array) {
				...
				this.send(
					[Enumerations.MIDI_SYSTEM_MESSAGES.sysex].concat(merged),
					{
						time: options.time,
					}
				);
		...
		sendSongPosition(value = 0, options = {}) {
			...
			var lsb = value & 0x7f;
			this.send(
				[Enumerations.MIDI_SYSTEM_MESSAGES.songposition, msb, lsb],
				{
					time: options.time,
				}
			);
		...
		sendModulationRange(semitones, cents, options = {}) {
			if (options.channels == undefined)
				options.channels = Enumerations.MIDI_CHANNEL_NUMBERS;
			Utilities.sanitizeChannels(options.channels).forEach(ch => {
				this.channels[ch].sendModulationRange(
					semitones,
					cents,
					options
				);
			...
			if (wm.validation) {
				...
				destinations.forEach(destination => {
					if (!(destination instanceof Output)) {
						throw new TypeError(
							"Destinations must be of type 'Output'."
						);
					}
				}); // Validate types
				...
				if (options.types !== undefined) {
					options.types.forEach(type => {
						if (
							!Enumerations.MIDI_SYSTEM_MESSAGES.hasOwnProperty(
								type
							) &&
							!Enumerations.MIDI_CHANNEL_MESSAGES.hasOwnProperty(
								type
							)
						) {
							throw new TypeError(
								"Type must be a valid message type."
							);
						}
					});
				} // Validate channels
				...
				if (options.channels !== undefined) {
					options.channels.forEach(channel => {
						if (
							!Enumerations.MIDI_CHANNEL_NUMBERS.includes(channel)
						) {
							throw new TypeError(
		...
		_dispatchParameterNumberEvent(type, paramMsb, paramLsb, e) {
			...
			if (type === "rpn") {
				event.parameter = Object.keys(
					...
					return (
						Enumerations.MIDI_REGISTERED_PARAMETERS[key][0] ===
							paramMsb &&
						Enumerations.MIDI_REGISTERED_PARAMETERS[key][1] ===
							paramLsb
					);
				});
			} else {
	...
	class Input extends EventEmitter {
		...
		hasListener(event, listener, options = {}) {
			...
			if (Enumerations.CHANNEL_EVENTS.includes(event)) {
				...
				if (options.channels === undefined)
					options.channels = Enumerations.MIDI_CHANNEL_NUMBERS;
				return Utilities.sanitizeChannels(options.channels).every(
					ch => {
		...
		removeForwarder(forwarder) {
			this._forwarders = this._forwarders.filter(
				item => item !== forwarder
			);
		}
		/**
	...
	class WebMidi extends EventEmitter {
		...
		async enable(options = {}, legacy = false) {
			...
			this.interface.onstatechange =
				this._onInterfaceStateChange.bind(this); // Update inputs and outputs (this is where `Input` and `Output` objects are created).
